...
class SaleOrderMySqlDAO implements SaleOrderDAO{
	...
	public function insert($saleOrder){
		$sql = 'INSERT INTO sale_order (parent_id, num_items_sold, total_sales, tax_total, shipping_total, net_total, status, success_indicator, payment_type, delivery_status, reference, customer_id, note, address_id, shipping_details, delivery_address, created_at, created_at_gmt, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
		$sqlQuery = new SqlQuery($sql);
		...
		$sqlQuery->set($saleOrder->shippingDetails);
	...
	public function update($saleOrder){
		$sql = 'UPDATE sale_order SET parent_id = ?, num_items_sold = ?, total_sales = ?, tax_total = ?, shipping_total = ?, net_total = ?, status = ?, success_indicator = ?, payment_type = ?, delivery_status = ?, reference = ?, customer_id = ?, note = ?, address_id = ?, shipping_details = ?, delivery_address = ?, created_at = ?, created_at_gmt = ?, updated_at = ? WHERE id = ?';
		$sqlQuery = new SqlQuery($sql);
	...
	public function queryByShippingDetails($value){
		$sql = 'SELECT * FROM sale_order WHERE shipping_details = ?';
		$sqlQuery = new SqlQuery($sql);
		$sqlQuery->set($value);
		return $this->getList($sqlQuery);
	}
	...
	public function deleteByShippingDetails($value){
		$sql = 'DELETE FROM sale_order WHERE shipping_details = ?';
		$sqlQuery = new SqlQuery($sql);
		$sqlQuery->set($value);
		return $this->executeUpdate($sqlQuery);
	}
	...
	protected function readRow($row){
		...
		$saleOrder->shippingDetails = $row['shipping_details'];
